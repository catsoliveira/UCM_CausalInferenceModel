...
class DataGenerator:
    ...
    def generate_uniformchannel(self, s):
    ...
    def generate_cyclicuniformchannel(self, s):
        matrix = np.zeros((s[0], s[1]))
        cond_prob =[]
        marg_prob=[]
        probabilities={}
    ...
    def getinstances(self, n, s):
        G = self.graph
        symmetric = self.generate_uniformchannel(s)
        #symmetric = self.generate_cyclicuniformchannel(s)
        print(symmetric[1])
        h = symmetric[0]
        instances=[]
        for m in range(n):
        ...
        return final_data, symmetric[1], h